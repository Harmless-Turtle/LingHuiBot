...
<h1 style="text-align:center;"><em>凌辉Bot 主系统功能列表</em></h1>
...
| ✅  | bug反馈     | `直接发送`  | 发送后将自动抓取日志并发送给管理员                            |

<h3>群老婆（？</h3>

| 可用 | 功能   | 语法示例/用法 | 效果                              |
|----|------|---------|---------------------------------|
| ✅  | 今日老婆 | `直接发送`  | 获取今日老婆，刷新周期为86400秒，每一个周期内可使用3次  |
| ✅  | 换老婆  | `直接发送`  | 换一个今日老婆，刷新周期为86400秒，每一个周期内可使用3次 |
...
此功能代码位于 <a href="../src/plugins/birthday">Project:Birthday</a> 中

⚠️警告⚠️<br>
此功能将默认您准许凌辉Bot记录`在您和凌辉Bot共同存在`
的群聊中的发送消息条数，以便实现凌辉仅在您生日前7天内最活跃的群聊发送生日祝贺，<em>凌辉Bot不会记录你的聊天信息。</em>

| 可用 | 功能                             | 语法示例/用法       | 效果                                                  |
|----|--------------------------------|---------------|-----------------------------------------------------|
| ❌  | 生日祝贺(开/关)                      | `生日祝贺开`       | 操作凌辉 Bot 是否需要启动生日祝贺功能<br><b><em>此功能仅群管理组可用</em></b> |
| ❌  | 我的生日是<月份数字(1-12)>-<日数数字(1-31)> | `我的生日是 01-10` | 更改自己的生日日期                                           |
| ❌  | 删除我的生日                         | `直接发送`        | 删除你记录的生日日期                                          |