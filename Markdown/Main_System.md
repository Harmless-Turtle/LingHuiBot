...
功能：功能名称。<br>
> 功能名字中的斜杠是或的意思<br>例如功能`签到/凌辉好久不见`的功能用法是`直接发送`，那么当您在使用签到这个功能时`直接发送`签到或者`直接发送`凌辉好久不见即可。<br>在功能名称中的“<>”符号意味着这是一个需要您传入参数的意思，例如`来只<数字sid>`功能需要您输入一个数字，以查找该数字的图片是否存在。
