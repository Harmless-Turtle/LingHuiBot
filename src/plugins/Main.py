...
async def ChekGroupMemberChange(event:GroupDecreaseNoticeEvent):
    try:
        Data = utils.handle_json(f'{path}/GroupMemberChange.json', "r")
...
async def add_group_switch(event:GroupRequestEvent):
    Data = utils.handle_json(f'{path}/add_group_switch.json', "r")
...
async def AWord_Function(matcher:Matcher,event=MessageEvent,args:Message = CommandArg()):
    ...
    Dict = utils.handle_json(AWord_Path, 'r')
...
async def Sign_in_Function(matcher:Matcher,event:MessageEvent,GroupEvent:GroupMessageEvent,args:Message=CommandArg()):
    ...
    AwordDict = utils.handle_json(AWord_Path, 'r')
...
async def Change_Exit_Function(matcher:Matcher,event:MessageEvent,args:Message=CommandArg()):
    ...
    utils.handle_json(f'{path}/GroupMemberChange.json', "w", Data)
...
async def LF_Function(matcher:Matcher,event:NoticeEvent):
    ...
    bot = get_bot()
    Data = event.model_dump()
    Dict = utils.handle_json(f"{path}/Friend_Like.json", 'r')
    ...
    Dict[f"{user_id}"]['Time'] = day
    utils.handle_json(f"{path}/Friend_Like.json", 'w', Dict)
    try:
...
async def utils_switch_add_group(matcher:Matcher,event:MessageEvent,args:Message=CommandArg()):
    args = str(args)
    Data = utils.handle_json(f'{path}/add_group_switch.json', 'r')
    if "开" in args:
        write = True
    ...
    utils.handle_json(f'{path}/add_group_switch.json', 'w', Data)
