...
async def add_group(
    ...
    if not group_id.isdigit():
        await add_group_blacklist.finish(
...
async def del_user(
    ...
    group_id = args.extract_plain_text().strip()
    if not group_id.isdigit():
        await del_group_blacklist.finish(
            MessageSegment.reply(event.message_id) +
            "唔...请输入一个正确的数字才可以呢。"
        )
...
async def add_user(
    ...
    if not user_id.isdigit():
        add_user_blacklist.finish(
            MessageSegment.reply(event.message_id) +
            "唔...请输入一个正确的数字才可以呢。"
        )

    if user_id == str(event.user_id):
        await add_user_blacklist.finish(
            MessageSegment.reply(event.message_id) +
...
async def del_user(
    ...
    obj = await session.get(UserBlacklist, user_id)
    if obj is None:
        await del_user_blacklist.finish(
            MessageSegment.reply(event.message_id) +
...
async def handle_check_user_blacklist(
    ...
    text = ""
    if not user_list:
        await check_user_blacklist.finish(
...
async def handle_check_group_blacklist(
    ...
    text = ""
    if not group_list:
        await check_group_blacklist.finish(
