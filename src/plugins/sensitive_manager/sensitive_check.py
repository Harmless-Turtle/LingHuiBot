...
class SensitiveManager:
    def __init__(self):
        ...
        self.group_settings = Handler.load_json(GROUP_SETTINGS_PATH, 'r') or {}
        # 直接加载新格式的违规记录
        self.group_violations = Handler.load_json(USER_VIOLATIONS_PATH, 'r') or {}
        
        # 初始化时遍历所有群组构建 AC 自动机
        self.build_all_ac()

        self.group_settings = Handler.load_json(GROUP_SETTINGS_PATH, 'r') or {}
        self.user_violations = Handler.load_json(USER_VIOLATIONS_PATH, 'r') or {}

    def load_data(self):
        data = Handler.load_json(SENSITIVE_DATA_PATH, 'r') or {}
        ...
        try:
            ...
            if Role not in "member":
                if warn_level == 1:
                    ...
                    Handler.load_json(USER_VIOLATIONS_PATH, 'w', manager.group_violations)
